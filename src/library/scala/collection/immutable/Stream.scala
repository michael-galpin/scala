...
self =>
   ...
   */
  override def ++[B >: A, That](that: Flattenable[B])(implicit bf: CanBuildFrom[Stream[A], B, That]): That =
   ...
   */
  override final def flatMap[B, That](f: A => Flattenable[B])(implicit bf: CanBuildFrom[Stream[A], B, That]): That =
  ...
  final class StreamWithFilter(p: A => Boolean) extends WithFilter(p) {    
    ...
    override def flatMap[B, That](f: A => Flattenable[B])(implicit bf: CanBuildFrom[Stream[A], B, That]): That = {
