...
trait SeqProxyLike[+A, +Repr <: SeqLike[A, Repr] with Seq[A]] extends SeqLike[A, Repr] with IterableProxyLike[A, Repr] { 
  ...
  override def union[B >: A, That](that: ThatSeq[B])(implicit bf: CanBuildFrom[Repr, B, That]): That = self.union(that)(bf)
  override def diff[B >: A](that: ThatSeq[B]): Repr = self.diff(that)
  override def intersect[B >: A](that: ThatSeq[B]): Repr = self.intersect(that)
